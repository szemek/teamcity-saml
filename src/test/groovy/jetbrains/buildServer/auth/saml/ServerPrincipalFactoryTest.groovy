...
class ServerPrincipalFactoryTest extends Specification {
    ...
    def "read user from model"() {
        given:
        def user = new SAMLUser("testUser")
        ...
        principal.realm == PluginConstants.SAML_AUTH_SCHEME_NAME
    }
    ...
    def "create user if model reports null"() {
        ...
        userModel.createUserAccount(PluginConstants.SAML_AUTH_SCHEME_NAME, user.id) >> teamcityUser
        when:
        ServerPrincipal principal = principalFactory.getServerPrincipal(user, true).get()
        then:
        principal != null
